...
async function findMatchingWorkflows(account, channel, text) {
...
function findTriggerNode(workflow, type, text) {
    const flow = workflow.flowDefinition;
    if (!flow || !flow.nodes)
        return null;
    ...
    return flow.nodes.find((n) => {
        if (n.type !== nodeType && n.type !== 'trigger')
            return false; // trigger is generic
...
async function startWorkflowExecution(workflow, account, senderId, nodeId, eventId, commentId) {
...
async function executeWorkflowNode(workflow, account, senderId, nodeId, runId, commentId) {
    const flow = workflow.flowDefinition;
    if (!flow)
        return;
    ...
    for (const edge of edges) {
        ...
        if (nextNode.type === 'instagram') {
            ...
            if (commentId && nextNode.data?.enableCommentReply) {
                ...
                await (0, instagram_api_1.replyComment)(account.accessTokenEncrypted, commentId, nextNode.data.commentReplyMessage);
            }
            if (commentId) {
                await (0, instagram_api_1.sendPrivateReply)(account.accessTokenEncrypted, commentId, text, imageUrl, buttons);
            }
            else {
                await (0, instagram_api_1.sendDM)(account.accessTokenEncrypted, senderId, text, imageUrl, buttons);
            }
            // Continue
            ...
            const ms = (0, utils_1.parseTimeToMs)(timeStr);
            await redis_1.instagramQueue.add('resumeWorkflow', {
                workflowId: workflow.id,
            ...
            const branchEdge = (flow.edges || []).find((e) => e.source === nextNode.id && e.sourceHandle === targetHandle);
            if (branchEdge) {
                const branchedNode = (flow.nodes || []).find((n) => n.id === branchEdge.target);
                if (branchedNode) {
